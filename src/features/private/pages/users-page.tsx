...
import { Trash2, Shield, ShieldAlert, Plus, Loader2, Link2, Edit2, ChevronLeft, ChevronRight, Search, XCircle } from 'lucide-react'; // Añadí Search y XCircle
...
export const UsersPage = () => {
    ...
    // 1. Filtrar usuarios primero
    const filteredUsers = useMemo(() => {
        if (!searchTerm) return users; // Si no hay búsqueda, devuelve todos
    ...
    // 2. Paginación sobre los usuarios YA FILTRADOS
    ...
    const handleCreateUser = async (userPayload: any) => {
        ...
        setStatusModal({ isOpen: true, type: 'success', title: 'Usuario Creado', description: result.message });
    };

    const handleUpdateUser = async (id: number, data: any) => {
        const result = await editUser(id, data);
        if (!result.success) throw new Error(result.message);
        setStatusModal({ isOpen: true, type: 'success', title: 'Actualizado', description: result.message });
    };

    const handleDeleteClick = async (id: number) => {
        if (!confirm('¿Eliminar usuario?')) return;
        const result = await removeUser(id);
        setStatusModal({ isOpen: true, type: result.success ? 'success' : 'error', title: result.success ? 'Eliminado' : 'Error', description: result.message });
    };

    const handleToggleRole = async (user: User) => {
        try { await switchRole(user); } catch { setStatusModal({ isOpen: true, type: 'error', title: 'Error', description: 'No se pudo cambiar el rol' }); }
    };
        ...
        <div className="flex flex-col h-full w-full p-4 md:p-8 overflow-y-auto bg-card-bg rounded-3xl border border-border-base shadow-lg">
            <div className="max-w-[1400px] mx-auto w-full space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                        ...
                        <div className="relative w-full sm:w-64">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-4 py-3 rounded-xl bg-bg-surface border border-border-base focus:ring-2 focus:ring-brand-primary/20 outline-none text-sm transition-all text-text-main"
                            />
                            {searchTerm && (
                                <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-brand-primary">
                                    <XCircle size={16} />
                                </button>
                            )}
                        </div>

                        <button onClick={() => setIsCreateModalOpen(true)} className="flex items-center justify-center gap-2 bg-brand-primary text-white px-5 py-3 rounded-xl font-bold shadow-lg shadow-brand-primary/20 hover:bg-brand-dark active:scale-95 transition-all whitespace-nowrap">
                ...
                {filteredUsers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 bg-bg-surface rounded-3xl border border-dashed border-border-base">
                        ...
                        <h3 className="text-lg font-bold text-text-main">No se encontraron resultados</h3>
                        <p className="text-text-muted text-sm">Intenta buscar con otro término.</p>
                        <button onClick={() => setSearchTerm('')} className="mt-4 text-brand-primary font-medium hover:underline">
                            Limpiar búsqueda
                        </button>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:hidden">
                            {paginatedUsers.map((u) => (
                                <div key={u.id} className="bg-bg-surface p-5 rounded-2xl shadow-sm border border-border-base flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    {/* ... (Contenido de Card Móvil igual que antes) ... */}
                                    ...
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <div className="flex flex-col gap-1 items-end">
                                            ...
                                            <button onClick={() => openAccounts(u)} className="flex items-center gap-1 text-brand-primary font-medium">
                                                <Link2 size={14} /> {u.connected_accounts?.length || 0} Vinculadas
                                            </button>
                                        </div>
                                    </div>
                        ...
                        {/* --- VISTA DESKTOP (TABLA) --- */}
                        <div className="hidden md:block rounded-3xl shadow-sm border border-border-base overflow-hidden bg-bg-surface">
                                ...
                                <tbody className="divide-y divide-border-base text-sm">
                                    {paginatedUsers.map((u) => (
                                        <tr key={u.id} className="hover:bg-bg-canvas/40 transition-colors group">
                                            {/* ... (Contenido de Fila Tabla igual que antes) ... */}
                                            ...
                                            <td className="p-5 text-center">
                                                <button onClick={() => handleToggleRole(u)} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border transition-transform active:scale-95 ${u.role === 'admin' ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-white text-gray-600 border-gray-200'}`}>
                                                    {u.role === 'admin' ? <ShieldAlert size={14} /> : <Shield size={14} />} {u.role.toUpperCase()}
                                                </button>
                                            </td>
                                            <td className="p-5 text-center">
                                                <button onClick={() => openAccounts(u)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-brand-primary bg-brand-primary/5 hover:bg-brand-primary/10 transition-colors border border-brand-primary/10">
                                                    <Link2 size={14} /> Ver Cuentas {(u.connected_accounts?.length || 0) > 0 && <span className="ml-1 bg-brand-primary text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full">{u.connected_accounts?.length}</span>}
                                                </button>
                                            </td>
                                            <td className="p-5 pr-8 text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                    <button onClick={() => openEdit(u)} className="p-2 rounded-lg text-text-muted hover:text-brand-primary hover:bg-brand-primary/10 transition-colors"><Edit2 size={18} /></button>
                                                    <button onClick={() => handleDeleteClick(u.id)} className="p-2 rounded-lg text-text-muted hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={18} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 py-2 text-sm text-text-muted border-t border-border-base pt-6">
                            <div className="flex items-center gap-2">
                                <span>Mostrar</span>
                                <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="bg-bg-surface border border-border-base rounded-lg px-2 py-1 focus:ring-2 focus:ring-brand-primary/20 outline-none">
                            ...
                            <div className="flex items-center gap-2 bg-bg-surface rounded-xl border border-border-base p-1">
                                <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-2 rounded-lg hover:bg-bg-canvas disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ChevronLeft size={18} /></button>
                                <span className="px-3 font-medium text-text-main">Página {currentPage} de {totalPages || 1}</span>
                                <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0} className="p-2 rounded-lg hover:bg-bg-canvas disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ChevronRight size={18} /></button>
                            </div>
                        </div>
                    </>
                )}
